...
{
    public class StaffValidator : AbstractValidator<StaffForm>
    {
        public StaffValidator()
        {
            ...
            RuleFor(m => m.Status)
                ...
                .Must(value => status.Contains(value))
                    .WithMessage("{PropertyName} is invalid.");
            When(m => m.Password != string.Empty && m.PasswordRepeat != string.Empty, () => {
        ...
        protected bool ValidateUniqueUsername(StaffForm form, string value)
        {
            try
            {
                User model = form.Model;
